...
async function fetchInventory() {
    const response = await fetch(`https://api2.kirka.io/api/${translations["inventory"]}`, {
        headers: {
...
function ingameShowcase(message, rarity, name) {
    ...
    document.getElementById(translations["notifications"]).children[0].appendChild(elem);
...
(async () => {
    ...
    let interval = setInterval(async () => {
        ...
        if (chestresult[translations["rarity"]]) {
            if (Object.keys(coloroutput).includes(translations[chestresult[translations["rarity"]]])) {
                ingameShowcase(
                ...
                console.log(chestresult[translations["rarity"]] + " " + chestresult[translations["name"]]);
            }
        } else {
        ...
        if (check == chestskipper.length * 5) {
            ...
            document.getElementById(translations["notifications"]).children[0].appendChild(endelem);
            setTimeout(() => {
                try {
                    endelem.remove();
                } catch {}
